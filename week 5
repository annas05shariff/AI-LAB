...
def calculate_cost(state):
    """Returns the number of attacking pairs of queens (heuristic cost)."""
    cost = 0
    n = len(state)
    for i in range(n):
...
def get_all_neighbors(state):
    """Generates all possible neighbors by swapping two queen positions."""
    neighbors = []
    n = len(state)
    for i in range(n):
        for j in range(i + 1, n):
...
def hill_climbing(state):
    """
    Solves the 4-Queens problem using steepest-ascent Hill Climbing.
    It evaluates all neighbors and picks the best one.
    """
    ...
    while True:
        ...
        # Sort by best cost, then by the smallest column pair as a tie-breaker
        ...
        # If the best neighbor is not better than the current state, we are stuck
...
def get_random_neighbor(state):
    """Generates a single random neighbor by swapping two queen positions."""
    n = len(state)
    neighbor = state.copy()
...
def simulated_annealing(state, initial_temp=100.0, cooling_rate=0.95, max_steps=1000):
    """
    Solves the 4-Queens problem using Simulated Annealing.
    It explores the search space by sometimes accepting worse moves.
    """
    ...
    for step in range(max_steps):
        ...
        # Always move if the neighbor is better
        if delta_cost < 0:
            print(f"Step {step+1}: Good move. Moved to {neighbor_state} (Cost: {neighbor_cost})")
            current_state = neighbor_state
            current_cost = neighbor_cost
        # If neighbor is worse, maybe move based on probability
        else:
